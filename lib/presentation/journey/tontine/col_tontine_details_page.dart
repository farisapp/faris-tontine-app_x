...
class ColTontineDetailsPage extends StatefulWidget {
  final Tontine tontine;

  const ColTontineDetailsPage({Key? key, required this.tontine}) : super(key: key);

  @override
  _TontineDetailsPageState createState() => _TontineDetailsPageState();
}

class _TontineDetailsPageState extends State<ColTontineDetailsPage> with WidgetsBindingObserver {
  ...
  void initState() {
    super.initState();
    WidgetsBinding.instance.addObserver(this); // <-- ICI : appel correct
    initJiffy();
    getData(false);
  }
  ...
  getData(bool reload) async {
    final controller = Get.find<TontineDetailsController>();

    await controller.getTontineDetails(widget.tontine.id, reload);
    await controller.getTontineMembres(widget.tontine.id, reload);
    await controller.getTontineRequetes(widget.tontine.id, reload);
    await controller.getTontinePeriodicites(widget.tontine.id, reload);
    await controller.getTontinePeriodicitesToPaid(widget.tontine.id, true);
    ...
    if (controller.tontine?.statut == "PENDING") {
      await controller.updateStatus(widget.tontine.id!, "RUNNING");
      await controller.getTontineDetails(widget.tontine.id, true);
    }

    controller.update(); // <=== Ajoute ça à la fin
  }
            ...
            if(detailController.tontine != null){
              ...
              return RefreshIndicator(
                ...
                child: SingleChildScrollView(
                  child: Column(
                    children: [
                      // Bouton pour partager l'épargne
                          ...
                          children: [
                                        ...
                                        if (detailController.tontine!.statut == "PENDING" && (detailController.membres != null)) ...[
                                          if (detailController.tontine?.createur?.id == userController.userInfo!.id)
                                            RoundTextButton(
                                              ...
                                              elevation: 2,
                                              onPressed: () async {
                                                // Lancer manuellement la cotisation sans confirmation
                                                await detailController.updateStatus(detailController.tontine!.id!, "RUNNING");
                                                await detailController.getTontineDetails(detailController.tontine!.id, true);
                                          ...
                                          if (detailController.tontine?.type == "EPARGNE") ...[
                                            ...
                                            // Ajoute ce bouton à côté du bouton "Retirer" existant dans ColTontineDetailsPage
                                            Wrap(
                                          ...
                                          Expanded(
                                            child: Center( // Centre le texte au milieu de l'écran
                                              child: Text(
                                                " Compte épargne clôturé, vous pouvez cliquer en haut à droite sur 'Supprimer' pour le supprimer. Merci",
                                                style: TextStyle(
                                                  color: Colors.red,
                                                  fontSize: 11,
                            ...
                            Row(
                              children: [
                                ...
                                SizedBox(width: 10),
                                // Bouton Clôturer ou Relancer selon le statut actuel
                                Expanded(
                                  child: detailController.tontine?.statut == "RUNNING"
                                    ...
                                    icon: Icons.close,
                                    onPressed: () async {
                                      // Seul l'organisateur (créateur) est autorisé à clôturer l'épargne
                                      final userId = userController.userInfo?.id;
                                      final createurId = detailController.tontine?.createur?.id;
                                      if (userId != createurId) {
                                        showCustomSnackBar(
                                          context,
                                          "C'est l'organisateur qui peut clôturer cette épargne.",
                                        );
                                        return;
                                      }
                                      ...
                                      bool? confirm = await Get.defaultDialog<bool>(
                                        ...
                                        middleText:
                                        "Êtes-vous sûr de vouloir clôturer cette épargne ? Cette action est définitive.",
                                        actions: [
                                          RoundTextButton(
                                            titre: "OUI",
                                            width: 80,
                                      ...
                                      if (confirm == true) {
                                        await detailController
                                          ...
                                          if (result.isSuccess) {
                                            showCustomSnackBar(
                                              context,
                                              "L'épargne a été clôturée avec succès.",
                                              isError: false,
                                            );
                                            await getData(true);
                                          } else {
                                      ...
                                      if (confirm == true) {
                                        await detailController
                                          ...
                                          if (result.isSuccess) {
                                            showCustomSnackBar(
                                              context,
                                              "L'épargne a été relancée avec succès.",
                                              isError: false,
                                            );
                                            await getData(true);
                                          } else {
                            ...
                            Row(
                              ...
                              children: [
                                ElevatedButton.icon(
                                  onPressed: () {
                                    final shareContent =
                                        "Bonjour.Je t'invite à participer à une épargne collective en ligne avec l'application Faris. Pour participer, merci de télécharger l'application sur Playstore (Android) ici: https://play.google.com/store/apps/details?id=com.powersofttechnology.faris ou sur AppStore (Iphone) ici: https://apps.apple.com/bf/app/faris/id6458983816?l=fr-FR . Le code épargne est : ${detailController.tontine?.numero ?? ""}. Economisons ensemble avec Faris!";
                                    Share.share(
                                      shareContent,
                                      subject: "Invitation à épargner",
                                    );
                                  },
                              ...
                              children: [
                                ElevatedButton.icon(
                                  onPressed: () async {
                                    bool? confirm = await Get.defaultDialog<bool>(
                                      title: "Rendre public",
                                      middleText: "Souhaitez-vous vraiment rendre cette épargne visible publiquement ?",
                                      actions: [
                                        RoundTextButton(
                                          titre: "OUI",
                                          width: 80,
                                    ...
                                    if (confirm == true) {
                                      ...
                                      if (result.isSuccess) {
                                        showCustomSnackBar(context, "L'épargne est maintenant publique.", isError: false);
              ...
              children: [
                ...
                if(tontine.statut == "PENDING")...[
                  if(Get.find<TontineDetailsController>().tontine?.createur?.id == Get.find<UserController>().userInfo!.id)...[
                    ...
                    RoundTextButton(
                        titre: "Quitter cette épargne collective",
                        backgroundColor: Colors.red,
                        textColor: Colors.white,
                        height: 35,
                        fontSize: 14,
            ...
            children: [
              RoundTextButton(
                ...
                onPressed: () async {
                  ...
                  showCustomSnackBar(
                    context,
                    "Vous avez quitté cette épargne collective avec succès.",
                    isError: false,
                  );
