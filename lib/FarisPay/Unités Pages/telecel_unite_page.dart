...
import 'Telecel_unite_payment_details_page.dart';

class TelecelUnitePage extends StatefulWidget {
  @override
  _TelecelUnitePageState createState() => _TelecelUnitePageState();
}

class _TelecelUnitePageState extends State<TelecelUnitePage> {
  ...
  Future<void> _savePlansLocally(List<Map<String, dynamic>> plans) async {
    ...
    prefs.setString('TelecelUnitePlans', json.encode(plans));
  }

  /// Récupère les forfaits stockés localement
  Future<List<Map<String, dynamic>>> _loadPlansFromLocal() async {
    final prefs = await SharedPreferences.getInstance();
    final plansJson = prefs.getString('TelecelUnitePlans');
    if (plansJson != null) {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonResponse['data'] != null && jsonResponse['data'] is List) {
          ...
          List<Map<String, dynamic>> fetchedPlans = data
              .where((plan) => plan['Operateur'] == 'Telecel' && plan['isAvailable'] == 1)
              .map((plan) {
  ...
  Future<void> _checkAndUpdateData() async {
    final prefs = await SharedPreferences.getInstance();
    String? lastUpdatedDate = prefs.getString('lastUpdatedDateTelecelUnite');
    ...
    if (lastUpdatedDate == null || lastUpdatedDate != todayDate) {
      ...
      prefs.setString('lastUpdatedDateTelecelUnite', todayDate);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          "Forfaits unités Telecel du $today",
                  ...
                  children: [
                    ...
                    ...entry.value.map((plan) => Card(
                      ...
                      child: ListTile(
                        ...
                        trailing: (plan['amount'] != null && plan['amount']! > 0)
                            ? Text(
                          "${plan['amount']} FCFA",
                          style: const TextStyle(
                            fontWeight: FontWeight.bold,
                            ...
                            : null, // Si le montant est 0 ou null, ne rien afficher
                        onTap: () {
                          ...
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => TelecelUnitePaymentDetailsPage(
