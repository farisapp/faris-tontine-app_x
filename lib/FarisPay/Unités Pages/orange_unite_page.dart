...
import 'Orange_unite_payment_details_page.dart';

class OrangeUnitePage extends StatefulWidget {
  @override
  _OrangeUnitePageState createState() => _OrangeUnitePageState();
}

class _OrangeUnitePageState extends State<OrangeUnitePage> {
  ...
  Future<void> _savePlansLocally(List<Map<String, dynamic>> plans) async {
    ...
    prefs.setString('OrangeUnitePlans', json.encode(plans));
  }

  /// Récupère les forfaits stockés localement
  Future<List<Map<String, dynamic>>> _loadPlansFromLocal() async {
    final prefs = await SharedPreferences.getInstance();
    final plansJson = prefs.getString('OrangeUnitePlans');
    if (plansJson != null) {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonResponse['data'] != null && jsonResponse['data'] is List) {
          ...
          List<Map<String, dynamic>> fetchedPlans = data
              .where((plan) => plan['Operateur'] == 'Orange' && plan['isAvailable'] == 1)
              .map((plan) {
  ...
  Future<void> _checkAndUpdateData() async {
    final prefs = await SharedPreferences.getInstance();
    String? lastUpdatedDate = prefs.getString('lastUpdatedDateOrangeUnite');
    ...
    if (lastUpdatedDate == null || lastUpdatedDate != todayDate) {
      ...
      prefs.setString('lastUpdatedDateOrangeUnite', todayDate);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          "Forfaits unités Orange du $today",
                      ...
                      child: ListTile(
                        ...
                        onTap: () {
                          ...
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => OrangeUnitePaymentDetailsPage(
